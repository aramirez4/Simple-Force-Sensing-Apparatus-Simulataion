function[Mov]=plotmove(A,expSched,target)
...
for i=1:length(movement)
    if expSched(i)==1;  % Determines which target based on experimental schedulae created from GenerateTarget.m (trgts)
        if target(i,3)==0;  % Checks if force field is applied 
        fname1=['M',num2str(check(1))];
        movDir1.(fname1)=movement{1,i};
        check(1)=check(1)+1;

        else
        fname1=['Mff',num2str(check(1))];  %% Force Field is applied
        movDir1.(fname1)=movement{1,i};
        check(1)=check(1)+1;
        end
    elseif expSched(i) ==2
